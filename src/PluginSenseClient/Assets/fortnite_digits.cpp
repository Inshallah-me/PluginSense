...
namespace
{
    ...
    bool DecodePngWic(const std::vector<unsigned char>& png, std::vector<unsigned char>& rgba, int& w, int& h)
    {
        if (png.empty())
            return false;

        const HRESULT co = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
        const bool coInitialized = SUCCEEDED(co);
        ...
        hr = converter->CopyPixels(nullptr, static_cast<UINT>(w * 4), static_cast<UINT>(rgba.size()), rgba.data());
        cleanup();
        return SUCCEEDED(hr);
    }

    ImTextureID UploadRgba(const std::vector<unsigned char>& rgba, int w, int h)
    {
        if (!g_device || rgba.empty() || w <= 0 || h <= 0)
    ...
    void LoadDigits()
    {
        if (g_loaded || g_failed || !g_device)
            return;

        for (int i = 0; i < 10; ++i)
        {
            const std::vector<unsigned char> png = DecodeBase64(kDigitPngBase64[i]);
            std::vector<unsigned char> rgba;
            int w = 0;
            int h = 0;
            if (!DecodePngWic(png, rgba, w, h))
            {
                g_failed = true;
                return;
            }

            g_digits[i].tex = UploadRgba(rgba, w, h);
            g_digits[i].w = w;
            g_digits[i].h = h;
            if (!g_digits[i].tex)
            {
...
namespace fortnite_digits
{
    ...
    ImTextureID Get(int digit, int& outW, int& outH) noexcept
    {
        outW = 0;
        outH = 0;
        if (digit < 0 || digit > 9 || !Ready())
            return (ImTextureID)nullptr;

        outW = g_digits[digit].w;
        outH = g_digits[digit].h;
        return g_digits[digit].tex;
    }
}
