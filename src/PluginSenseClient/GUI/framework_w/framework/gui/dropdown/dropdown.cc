...
namespace framework
{
	...
	void c_dropdown::draw()
	{
		...
		animations::m_dropdown_value = utils::builder::create_animation_ctx(m_parent + m_label + "#m_dropdown_value", m_visible && (this->m_items[*this->m_val] != "None") || g_ctx->is_focused(this) && g_ctx->m_open, 0.5f);
		animations::m_dropdown_hover = utils::builder::create_animation_ctx(m_parent + m_label + "#m_dropdown_hover", m_visible && g_ctx->m_hovered == this, 0.5f);
		animations::m_dropdown_open = utils::builder::create_animation_ctx(m_parent + m_label + "#m_dropdown_open", m_visible && g_ctx->is_focused(this), 0.5f);

		// animation handling
		float target_opacity = 0.2f;
		if (animations::m_dropdown_value.val() > 0.f)
			target_opacity = 0.2f + (0.6f * animations::m_dropdown_value.val());
		...
		g_render->use_layer(m_layer, [&]()
			{
				...
				// value selected:图标最左,枪名右对齐
				const auto selected_text_color = g_style->m_text.modulate(animations::m_window_opacity.limit(0.3f).val());
				if ( m_icon_stacks )
				{
					const auto icons = m_icon_stacks();
					if ( *this->m_val >= 0 && *this->m_val < static_cast<int>( icons.size() ) && !icons[ *this->m_val ].empty() )
						g_font->f_weapon_icons.text( (m_pos + position).x + 6.f , (m_pos + position).y + 3.f , icons[ *this->m_val ] , selected_text_color );
				}
				const float selected_base_x = m_icon_stacks ? 60.f : 6.f;
				g_font->f_childs.text( (m_pos + position).x + selected_base_x , (m_pos + position + math::c_vector_2d(5, 3)).y, this->m_items[*this->m_val], selected_text_color);
		...
		if (animations::m_dropdown_open.val() > 0.f)
		{
			...
			auto calculated_size = math::c_vector_2d(m_child_size, std::min((this->m_items.size() * g_font->f_childs.measure(m_label).y + 1) + 6, k_max_list_height));

			const float row_h = g_font->f_childs.measure(m_label).y;
			const float visible_height = calculated_size.y - 6.f;
			const float total_height = row_h * this->m_items.size();
			...
			g_render->use_layer(engine::render_layer::prioritized, [&]()
				{
					...
					for (int i = 0; i < this->m_items.size(); i++)
					{
						...
						animations::m_dropdown_switch = utils::builder::create_animation_ctx(dropdown + std::to_string(i), (*this->m_val == i) && g_ctx->is_focused(this), 0.5f);
						animations::m_dropdown_option_hover = utils::builder::create_animation_ctx(dropdown + std::to_string(i) + "anim", g_input->mouse_in_region(bounding_switch.pos(), bounding_switch.size()) && g_ctx->is_focused(this), 0.5f);
						...
						{
							const auto icons = m_icon_stacks();
							if ( i >= 0 && i < static_cast<int>( icons.size() ) && !icons[ i ].empty() )
								g_font->f_weapon_icons.text( m_pos.x + position2.x + 5.f + (5.f * animations::m_dropdown_option_hover.val()) , y , icons[ i ] , option_text_color );
						}
						const float option_base_offset = m_icon_stacks ? 55.f : 5.f;
						g_font->f_childs.text( m_pos.x + position2.x + option_base_offset + (5.f * animations::m_dropdown_option_hover.val()) , y, dropdown, option_text_color);
