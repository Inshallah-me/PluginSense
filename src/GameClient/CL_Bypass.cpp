...
auto CL_Bypass::PostClientCreateMove( CCSGOInput* pCCSGOInput , CUserCmd* pUserCmd ) -> void
{
	...
	const auto OriginalCrcBase64 = base64( (unsigned char*)OriginalCrc.c_str() , static_cast<int>( OriginalCrc.size() ) , &EncodeLen );

	const auto SpoofedOriginalCrc = SpoofCrc();
	const auto SpoofedOriginalCrcBase64 = base64( (unsigned char*)SpoofedOriginalCrc.c_str() , static_cast<int>( SpoofedOriginalCrc.size() ) , &EncodeLen );
