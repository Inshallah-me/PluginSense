# PluginSense [Join Discord](https://discord.gg/jqmZvefGfY)

Lightweight CS2 plugin made for double injecting with [GameSense](https://gamesense.pub).

## Build

- Visual Studio 2022 (MSVC 14.44), Release x64
- Open `PluginSense.sln` and build, output: `bin\x64\Release\PluginSense.dll`

## Credits

- [Andromeda-CS2-Base](https://github.com/or75/Andromeda-CS2-Base)
- [Velocity](https://github.com/wubly/velocity)
- [Vesta](https://github.com/Read1dno/vesta)
- [Electro GUI](https://www.unknowncheats.me/forum/counter-strike-2-a/761976-gui-framework-electro.html)
- [Helper data](https://e-z.bio/270)
